...
namespace file::listing {
  ...
  public:
    ...
    virtual void redis_subscribe(const std::string& dname, void* opaque) = 0;
    virtual void redis_publish(const std::string& dname, const std::string& rex_attr) = 0;
    virtual ~Notify()
      {}
  }; /* Notify */
  ...
  public:
    ...
    virtual void redis_subscribe(const std::string& dname, void* opaque) override {}
    virtual void redis_publish(const std::string& dname, const std::string& rex_attr) override {}
...
#if defined(BOOST_ASIO_HAS_CO_AWAIT)
  class Redis_Notify : public Notify
  {
    ...
    Redis_Notify(Notifiable* n, const std::string& bucket_root)
      : Notify(n, bucket_root),
        thrd(&Redis_Notify::receiver, this)
        {
        initialize();
        }
  ...
  public:
    ...
    virtual void redis_subscribe(const std::string& dname, void* opaque) override {
      std::cout << "This has entered Redis watch" << std::endl;
    ...
    virtual void redis_publish(const std::string& dname, const std::string& rex_attr) override {
      ...
      co_publish(dname, rex_attr);
    }

    virtual int add_watch(const std::string& dname, void* opaque) override {return 0;}
    virtual int remove_watch(const std::string& dname) override {return 0;}

    virtual ~Redis_Notify() {
      shutdown();
      thrd.join();
    }
  };
