...
def connection():
    ...
    #default_endpoint = "%s://%s:%d" % (proto, hostname, port)

    conn = S3Connection(aws_access_key_id=vstart_access_key,
	    	      aws_secret_access_key=vstart_secret_key,
                      is_secure=False, port=8000, host=hostname, calling_format='boto.s3.connection.OrdinaryCallingFormat')

    #conn.is_secure = False
    #conn.host = hostname
    #conn.port = port
    #conn.aws_access_key_id = vstart_access_key
    #conn.aws_secret_access_key = vstart_secret_key
...
def test_ps_s3_persistent_gateways_recovery():
    ...
    topic_arn1 = topic_conf1.set_config()
    topic_conf2 = PSTopicS3(gw2.connection, topic_name+'_2', zonegroup, endpoint_args=endpoint_args+'&OpaqueData=fromgw2')
    topic_arn2 = topic_conf2.set_config()
...
def test_ps_s3_persistent_multiple_gateways():
    ...
    topic_conf2 = PSTopicS3(gw2.connection, topic_name+'_2', zonegroup, endpoint_args=endpoint_args+'&OpaqueData='+topic2_opaque)
    topic_arn2 = topic_conf2.set_config()
